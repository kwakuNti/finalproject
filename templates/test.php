    ...
    document.getElementById('searchButton').addEventListener('click', function(event) {
        ...
        xhr.onload = function() {
            if (xhr.status >= 200 && xhr.status < 400) {
                ...
                if (response.success) {
                    // Construct flight details message
                    var flightDetails = response.message;

                    // Display SweetAlert pop-up with flight details and buttons
                    Swal.fire({
                        title: 'Flight Details',
                        html: flightDetails,
                        icon: 'info',
                        showCancelButton: true,
                        confirmButtonText: 'Book Flight',
                        cancelButtonText: 'Cancel',
                        showLoaderOnConfirm: true,
                        preConfirm: () => {
                            // Redirect to book flight page
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($searchResults)) {
        ...
        foreach ($searchResults as $flight) {
            // Assuming your flight details are stored in $flight array
            // Append flight details to the message string
            $message .= "Flight Code: " . $flight['flight_code'] . ", Departure: " . $flight['origin_destination_name'] . ", Destination: " . $flight['destination_destination_name'] . ", Departure Time: " . $flight['departure_datetime'] . ", Arrival Time: " . $flight['arrival_datetime'] . "\n";
        }
